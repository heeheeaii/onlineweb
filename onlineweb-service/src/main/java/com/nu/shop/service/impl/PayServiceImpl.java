...
import com.nu.shop.bean.enums.PayType;
import com.nu.shop.bean.event.PaySuccessOrderEvent;
...
import com.nu.shop.service.PayService;
...
public class PayServiceImpl implements PayService {
    ...
    public List<String> paySuccess(String payNo, String bizPayNo) {
        ...
        if (settlement.getPayStatus() == 1) {
            throw new OnlineWebException("订单已支付");
        }
        // 修改订单结算信息
        if (orderSettlementMapper.updateToPay(payNo, settlement.getVersion()) < 1) {
            throw new OnlineWebException("结算信息已更改");
        }
