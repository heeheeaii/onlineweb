...
import com.nu.shop.bean.model.UserAddrOrder;
import com.nu.shop.bean.param.DeliveryOrderParam;
import com.nu.shop.bean.param.OrderParam;
...
public class OrderController {
    ...
    public ServerResponseEntity<Order> info(@PathVariable("orderNumber") String orderNumber) {
        ...
        if (!Objects.equal(shopId, order.getShopId())) {
            throw new OnlineWebException("您没有权限获取该订单信息");
        }
    ...
    public ServerResponseEntity<Void> delivery(@RequestBody DeliveryOrderParam deliveryOrderParam) {
        ...
        if (!Objects.equal(shopId, order.getShopId())) {
            throw new OnlineWebException("您没有权限修改该订单信息");
        }
