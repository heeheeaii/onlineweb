...
import com.nu.shop.security.admin.util.SecurityUtils;
import com.nu.shop.security.common.enums.SysTypeEnum;
import com.nu.shop.security.common.manager.PasswordManager;
import com.nu.shop.security.common.manager.TokenStore;
import com.nu.shop.sys.constant.Constant;
import com.nu.shop.sys.dto.UpdatePasswordDto;
import com.nu.shop.sys.model.SysUser;
import com.nu.shop.sys.service.SysRoleService;
import com.nu.shop.sys.service.SysUserService;
...
public class SysUserController {
	...
	public ServerResponseEntity<String> password(@RequestBody @Valid UpdatePasswordDto param){
        ...
        if (Objects.equals(1L,userId) && StrUtil.isNotBlank(param.getNewPassword())) {
            throw new OnlineWebException("禁止修改admin的账号密码");
        }
...
//		更新密码
	...
	public ServerResponseEntity<SysUser> info(@PathVariable("userId") Long userId){
		...
		if (!Objects.equals(user.getShopId(), SecurityUtils.getSysUser().getShopId())) {
			throw new OnlineWebException("没有权限获取该用户信息");
		}
		//获取用户所属的角色列表
	...
	public ServerResponseEntity<String> update(@Valid @RequestBody SysUser user){
		...
		if (!Objects.equals(dbUser.getShopId(), SecurityUtils.getSysUser().getShopId())) {
			throw new OnlineWebException("没有权限修改该用户信息");
		}
		...
		if (Objects.equals(1L,user.getUserId()) && user.getStatus()==0) {
			throw new OnlineWebException("admin用户不可以被禁用");
		}
