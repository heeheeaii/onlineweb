package com.nu.shop.common.util;
...
public class ImgUploadUtil {
    ...
    public Integer getUploadType() {
        ...
        if (Objects.isNull(uploadType)) {
            throw new OnlineWebException("请配置图片存储方式");
        }
        return uploadType;
    }
    ...
    public String getUploadPath() {
        ...
        if (Objects.isNull(imagePath) || StrUtil.isBlank(imagePath)) {
            throw new OnlineWebException("请配置图片存储路径");
        }
        return imagePath;
    }
    ...
    public String getResourceUrl() {
        ...
        if (Objects.isNull(resourceUrl) || StrUtil.isBlank(resourceUrl)) {
            throw new OnlineWebException("请配置图片路径");
        }
        return resourceUrl;
    }
    ...
    public String upload(MultipartFile img, String fileName) {
        ...
        if (!file.exists()) {
            ...
            if (!result) {
                throw new OnlineWebException("创建目录：" + filePath + "失败");
            }
        }
        try {
            img.transferTo(file);
        } catch (IOException e) {
            throw new OnlineWebException("图片上传失败");
        }
        return fileName;
    }
